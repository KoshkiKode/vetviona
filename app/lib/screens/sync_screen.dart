...
class _SyncScreenState extends State<SyncScreen> {
        ...
        actions: [
          ...
          if (sync.isServerRunning)
            ...
            TextButton.icon(
              icon: Icon(Icons.play_circle_outlined, color: cs.onPrimary),
              label:
                  Text('Go Online', style: TextStyle(color: cs.onPrimary)),
              onPressed: () => sync.startServer(),
            ),
        ],
      ),
        ...
        children: [
          ...
          _SyncCard(
            ...
            children: [
              ...
              FilledButton.icon(
                icon: const Icon(Icons.add_link),
                label: const Text('Generate Pairing Code'),
                onPressed: () => _generatePairingCode(context, tree, sync),
              ),
            ],
          ),
          const SizedBox(height: 12),
          ...
          _SyncCard(
            ...
            children: [
              ...
              OutlinedButton.icon(
                ...
                label: const Text('Pair'),
                onPressed: _secretController.text.trim().isNotEmpty
                    ? () => _pairWithCode(context, tree, sync)
                    : null,
                onPressedNotify: _secretController,
              ),
            ],
          ),
          const SizedBox(height: 12),
          ...
          if (sync.isServerRunning) ...[
            ...
            _SyncCard(
              ...
              title: 'Server Info',
              children: [
                _LabeledRow(label: 'Port', value: '${sync.serverPort}'),
                const SizedBox(height: 4),
                Text(
                  'Other devices can connect to this device using your local IP address and the port shown above.',
                  style: Theme.of(context).textTheme.bodySmall?.copyWith(
                      color: cs.onSurfaceVariant),
                ),
              ],
  ...
  Future<void> _generatePairingCode(
    ...
    // Create a new device record whose sharedSecret IS the pairing code.
    final device = Device.create(tier: tree.currentAppTierString);
    await tree.addDevice(device);
    if (!context.mounted) return;
    ...
    await showDialog<void>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text(
              'Share this code with the other device. They should enter it under "Pair with Code".',
              style: Theme.of(ctx).textTheme.bodySmall,
            ),
            const SizedBox(height: 16),
            Container(
            ...
            const SizedBox(height: 8),
            Text(
              'This code will only work once — a new code is generated each time you tap this button.',
              style: Theme.of(ctx).textTheme.bodySmall?.copyWith(
                    color: cs.onSurfaceVariant,
                    fontSize: 11,
                  ),
  ...
  Future<void> _pairWithCode(
    ...
    // Check if we're already paired with this secret.
    if (tree.pairedDevices.any((d) => d.sharedSecret == secret)) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Already paired with this code.')),
      );
    ...
    // Store a placeholder device; the real device ID is resolved on first sync.
    final device = Device(
      id: 'pending_${DateTime.now().millisecondsSinceEpoch}',
      sharedSecret: secret,
      tier: 'mobileFree',
    ...
    if (context.mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text(
                'Device paired. Use "Sync Now" or wait for auto-discovery.')),
      );
    }
  }
  ...
  Future<void> _manualSync(BuildContext context, SyncService sync) async {
    ...
    if (host.isEmpty || portText.isEmpty || secret.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please fill in host, port, and code.')),
      );
      return;
    }
...
class _PeerTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final cs = Theme.of(context).colorScheme;
    final isPaired = treeProvider.pairedDevices
        .any((d) => d.id == peer.deviceId);

    Device? pairedDevice;
    if (isPaired && peer.deviceId != null) {
      pairedDevice = treeProvider.pairedDevices
          .where((d) => d.id == peer.deviceId)
          .firstOrNull;
    }
    ...
    return ListTile(
      ...
      leading: CircleAvatar(
        backgroundColor: cs.primaryContainer,
        child: Icon(Icons.devices, color: cs.onPrimaryContainer, size: 18),
      ),
      title: Text(peer.name),
      subtitle: Text(
        '${peer.host}:${peer.port}  •  ${_tierLabel(peer.tier)}',
      ...
      trailing: isPaired && pairedDevice != null
          ? FilledButton.tonalIcon(
              icon: const Icon(Icons.sync, size: 16),
              label: const Text('Sync'),
              onPressed: () => syncService.syncWithPeer(
  ...
  Future<void> _showPairDialog(BuildContext context) async {
    ...
    // Store device and immediately sync.
    final alreadyPaired = treeProvider.pairedDevices
        .any((d) => d.sharedSecret == confirmed);
    if (!alreadyPaired) {
      final device = Device(
        id: peer.deviceId ?? 'pending_${DateTime.now().millisecondsSinceEpoch}',
        sharedSecret: confirmed,
        tier: peer.tier ?? 'mobileFree',
      );
      await treeProvider.addDevice(device);
...
class _PairedDeviceTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ListTile(
      ...
      leading: CircleAvatar(
        backgroundColor:
            discovered != null ? cs.primaryContainer : cs.surfaceContainerHighest,
        child: Icon(
          Icons.devices,
          color: discovered != null
              ? cs.onPrimaryContainer
