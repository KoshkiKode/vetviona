...
class OnThisDayScreen extends StatelessWidget {
  ...
  List<_OnThisDayEvent> _gatherEvents(
    ...
    for (final partnership in partnerships) {
      if (partnership.marriageDate != null &&
          partnership.marriageDate!.month == month &&
          partnership.marriageDate!.day == day) {
        ...
        if (p1 != null && p2 != null) {
          final yearsAgo = DateTime.now().year - partnership.marriageDate!.year;
          events.add(_OnThisDayEvent(
            type: _EventType.marriage,
            personId: p1.id,
            personName: '${p1.name} & ${p2.name}',
            year: partnership.marriageDate!.year,
            yearsAgo: yearsAgo,
            place: partnership.marriagePlace,
